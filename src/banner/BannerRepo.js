...
export async function removeBanner({ images = [], image }) {
  ...
  try {
    await runTransaction(db, async (transaction) => {
      ...
      console.log("Updating document with images:", images.filter((e) => e.name !== image)); // Add this line
      transaction.update(docRef, {
        images: images.filter((e) => e.name !== image),
      });

      try {
        console.log("Deleting storage object:", bannersRef); // Add this line
...
export async function getBanners() {
  ...
  if (snapShot.get("images").length>0) {
...
//   const docRef = doc(db, "banners", homeBannerId);
//   const fileName = `${Timestamp.now().toMillis()}${getFileExtension(file.name)}`;
//   const bannersRef = ref(storage, `banners/${fileName}`);
...
//       const currentImages = bDoc.data().images || []; // Ensure images is initialized as an array
//       transaction.update(docRef, {
//         images: [...currentImages, { name: fileName }],
//       });
...
//   return [...(snapShot.exists() ? snapShot.get("images") || [] : [])]; // Ensure to return an array even if images are not present
// }
