...
function ProductSetting() {
  ...
  useEffect(() => {
    ...
    if (setting.status === Actions.success) {
      let payLoad = setting.payload;
      setPublisherList(Array.isArray(payLoad.publishers) ? payLoad.publishers : []);
      setEditionList(Array.isArray(payLoad.editions) ? payLoad.editions : []);
    }
  }, [setting]);
      ...
      <Modal title="Add Publisher" isOpen={showPublisherDialog}>
        <PublisherEditDialog
          handleClose={(value) => {
            setShowPublisherDialog(false);
            if (value && value.length !== 0) {
              setPublisherList((prev) => [...prev, value]);
            }
          }}
          list={publisherList}
        />
      ...
      <Modal title="Add Edition" isOpen={showEditionDialog}>
        <EditionEditDialog
          handleClose={(value) => {
            setShowEditionDialog(false);
            if (value && value.length !== 0) {
              setEditionList((prev) => [...prev, value]);
            }
          }}
          list={editionList}
        />
          ...
          <Card>
            <Card.Body>
              <div className="d-flex flex-col">
                <div className="mb-3">
                  <div className="flex items-center">
                    <h6 className="text-lg font-bold text-gray-900">Publishers</h6>
                    <PrimaryOutlineButton
                      className="ml-auto"
                      small={true}
                ...
                <div>
                  <div className="flex items-center">
                    <h6 className="text-lg font-bold text-gray-900">Editions</h6>
                    <PrimaryOutlineButton
                      className="ml-auto"
                      small={true}
                      onClick={() => setShowEditionDialog(true)}
                    >
                      <PlusIcon className="w-4 h-4 mr-2" />
                      Add New
                    </PrimaryOutlineButton>
            ...
            <Card.Footer>
              <div className="flex">
                <PrimaryButton
                  className="ml-auto"
                  type="submit"
                  disabled={state.status === Actions.loading || setting.status === Actions.loading}
                  loading={state.status === Actions.loading}
                >
                  Save
                </PrimaryButton>
